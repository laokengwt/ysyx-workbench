...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(rd) = (src2 == 0) ? -1 : ((src1 == INT32_MIN && src2 == -1) ? INT32_MIN : (sword_t)src1 / (sword_t)src2));
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(rd) = (src2 == 0) ? -1 : src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(rd) = (src2 == 0) ? src1 : ((src1 == INT32_MIN && src2 == -1) ? 0 : src1 - (sword_t)src1 / (sword_t)src2 * src2));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc; s->dnpc = src1 + imm;
    ...
    } else if (rd == 0 && imm == 0) { // Tail Call
      // I choose to skip it to handle the Tail Call Optimization
      // trace_func_call(s->pc, s->dnpc);
    }
  );
...
int isa_exec_once(Decode *s) {
  ...
  IFDEF(CONFIG_ITRACE, store_inst(s->pc, s->isa.inst));
